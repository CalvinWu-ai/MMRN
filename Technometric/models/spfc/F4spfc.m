...
if u==r,
    ...
    f = n*p/2*log(2*pi) + n/2*logdet(D) + n/2*sum(aur);            
elseif u==0,
      [vecs,vals] = firsteigs(inv(D)*A,p);
      f = n*p/2*log(2*pi) + n/2*logdet(D) + n/2*sum(vals);    
else
    [vau,au] = firsteigs(auxf,p);
    [vaur,aur] = firsteigs(auxr,p); 
    f = n*p/2*log(2*pi) + n/2*logdet(D) + n/2*sum(aur) + n/2*sum(au((u+1):min(r,p)));
end
