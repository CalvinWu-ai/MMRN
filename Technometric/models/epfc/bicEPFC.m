...
for u=0:umax,
    if u==0,
        ...
        if ~isempty(parameters.sg),
            fu(u+1) = sg_min(Fhandle,dFhandle,Wo,parameters.sg{:},parameters.maxiter);
        else
            fu(u+1) = sg_min(Fhandle,dFhandle,Wo,'prcg','euclidean',{1:u},'quiet',parameters.maxiter);
        end
    end
    % degrees of freedom for current u
    dofu(u+1) = dof(u); 
end
% test
crit = 2*fu + log(n)*dofu;
d = argmin(crit);
d = d-1; % correction because of the inclusion of u=0;
