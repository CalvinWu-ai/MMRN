...
%
% Inputs:
%    - valsAfit: ordered eigenvalues of the residual covariance matrix from
%    the regression of the centered predictors onto a basis fy.
...
if u==0,
    f = n*p/2 + n*p/2*log(sum(valsA)/p); 
else
    f = n*p/2 + n*p/2*log((sum(valsA)-sum(valsAfit))/p);
end
