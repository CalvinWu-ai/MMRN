...
function pushDATA_Callback(hObject, eventdata, handles)
    ...
    % BORRAR: inicializa popups segun los datos
    %---------------------------------------------------------
    ...
    % BORRAR: carga predictores....lo pasaría a otro lado
...
function pushRUN_Callback(hObject, eventdata, handles)
            ...
            % MESSAGES AND GUI UPDATE            
            current_test = strcat(handles.model,'_',int2str(dim));
...
function edit_h_Callback(hObject, eventdata, handles)
    ...
    if strcmp(model,'PFC')||strcmp(model,'IPFC')||strcmp(model,'SPFC')||strcmp(model,'SEPFC'),
        ...
        disp(strcat('WARNING: the regression matrix for model ',model,' was built'));
        disp(' using data considered as the response when loading data from a file.')
        disp('If you change the response, please notice that this matrix will be wrong');
        handles.optargs = [];
...
function [] = save_results(handles)
    ...
    for k=1:length(idx_selected),
        ...
        [file,path] = uiputfile(strcat(current_model,'-results.txt'),strcat('Save results of ',current_model,' as'));
